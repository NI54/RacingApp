...
//var urlVersion="http://localhost/server/version.php";
	...
	function Clikeado(title,date,imagen,articleNumber){
		...
		$.get("http://www.ni54.com/Racing/proxy.php?url="+urlArt+articleNumber+'.txt', function( data ) {
		//$.get( urlArt+articleNumber+'.txt', function( data ) {
			...
			$( '#nota' ).html("");
			$( '#nota' ).append('<div style="width:30%;height:12em;float:left;" ><img src="http://localhost/server/'+imagen+'" style="width:100%;height:auto;border:1px solid black;top:0;border-radius:10px;"></div>');
